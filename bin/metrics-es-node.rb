...
class ESMetrics < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def get_es_resource(resource)
    ...
    r = RestClient::Resource.new("#{protocol}://#{config[:host]}:#{config[:port]}#{resource}", verify_ssl: false, user: config[:shield_user], password: config[:shield_password], verify_ssl: false, timeout: config[:timeout], headers: headers)
    JSON.parse(r.get)
